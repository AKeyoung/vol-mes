        ...
        >
          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            <el-autocomplete
                  ...
                  :disabled="item.readonly || item.disabled"
                  value-key="display"
                  v-model="formFields[item.field + 'temp']"
                  :autosize="{
                    minRows: item.minRows || 2,
                    maxRows: item.maxRows || 10,
                  }"
                  ...
                  @select="
                    (v) => {
                      handleSelect(formFields, item, v);
                    }
                  "
                  @change="
                    (v) => {
                      handleChange(formFields, item, v);
                    }
                  "
                >
                  <template #suffix>
...
export default defineComponent({
  ...
  methods: {
    ...
    getRule(item, formFields) {
      ...
      if(item.type == 'remoteSearch'){
        return {
          validator: (rule, val, callback) => {
            if(item.required && !this.isReadonly(item) && !val){
                return callback(new Error(item.title + '不能为空'));
            } else{
            return callback();}
          },
