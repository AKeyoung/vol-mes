...
namespace VOL.System.Services
{
    public partial class Sys_DictionaryService
    {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public object GetSearchRemote(string dicNo, string value)
        {
            Console.Write("调用远程搜索");
            try
            {
                ...
                // lsy 新增远程搜索
                var dicEntity = DBServerProvider.DbContext
                        .Set<Sys_Dictionary>()
                        .FirstOrDefault(x => x.DicNo == dicNo);

                if (dicEntity == null) return null;

                var sql = DictionaryHandler.GetCustomDBSql(dicNo, dicEntity.DbSql);
                if (string.IsNullOrEmpty(sql))
                {
                    return new List<object>();
                }

                return repository.DapperContext.QueryList<object>(sql, new { qs = "%" + value + "%" });
            }
            catch (Exception e)
            {
